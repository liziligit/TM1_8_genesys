...
int stopMode(char *dataType = "pd2", const int dataID = 0){
    // char *input = Form("../out%d.%s", dataID, dataType);
    // char *output = Form("adcSpectrum_ZS_%s_%d.root", dataType, dataID);
	char *input = Form("C:\\Users\\Administrator\\Desktop\\util\\out%d.%s", dataID, dataType);
    char *output = Form("C:\\Users\\Administrator\\Desktop\\util\\adcSpectrum_ZS_%s_%d.root", dataType, dataID);
    ...
    int pid = strcmp(dataType, "pd2");
    if(pid==0) {
        cout << "Data type " << pid +2<< endl;
        
        placData_2 pd2;
        pd2.read(input);
        
        const int nChips   = (int)pd2.adcCha();
        // const int nPixels  = (int)pd1.nPix();
        // const int nSamples = (int)pd1.samPerPix();
        // const int nFrames  = (int)pd1.nFrame();
        const int nPixels=5184;
		const int nSamples=2;
		int totalSample=pd2.dataSize()/sizeof(unsigned short);
		const int nFrames=totalSample/5184/nChips/nSamples;
        ...
        unsigned short *ps = (unsigned short *)pd2.p;
        ...
        for(int iChip=0; iChip<nChips; iChip++) { //0 - 8
            ...
            for(int iPixel=0; iPixel<nPixels; iPixel++) { //0 - 5184
                ...
                for(int iFrame=0; iFrame<nFrames; iFrame++) { //0 - 1617
                    ...
                    histAdc->SetBinContent( iFrame+1, int(adcValue/nSamples - 445) );
